...
class UploadSelf extends React.Component{
  ...
  componentWillMount(){
    ...
    if(this.props.uploadParameter.disabled){
      this.setState({
        display: 'block',
        iconDisplay: "none"
      })
  ...
  onChange = (info) => {
    ...
    if(info.file.status === "done"){
      ...
      this.setState({
        display: 'block',
        iconDisplay: "none",
        bgImg: info.file.response.url
      })
      this.props.uploadParameter.success(info,this.props.valueKey);
    } else if(info.file.status === "error"){
      message.error("上传失败！");
      this.props.uploadParameter.fail(info);
    }
  }
  render(){
    const bgImg=this.props.bgImg||this.state.bgImg;
    return(
      <Dragger {...this.props.uploadParameter.props}
        onChange={this.onChange}
        showUploadList={false}
        disabled={this.props.uploadParameter.disabled}>
        <div style={{padding: '40px 0', position: 'relative'}}>
          <Row gutter={16}>
            <Col span={24} className={less.iconBox}>
              <i className={less.icon} style={{backgroundImage: "url("+this.imgSrc+")", display: this.state.iconDisplay}}></i>
              <img src={bgImg} style={{width: '100%', display: this.state.display}}></img>
            </Col>
          </Row>
          <Row gutter={16}>
            <Col span={24}>
