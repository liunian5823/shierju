...
class InquiryManagement extends React.Component {
    ...
    columns = [{
        ...
        sorter:true,
        render:(text,record) => {
            if(record.status==10002 && record.quotationCount<=0){
                return "询价中"
            }else {
        ...
        render: (text, record) => {
            ...
            return(
                <span>
                    <a onClick={()=>{this.seeDetails(record.uuids)}}>查看</a>
                    {showTypeA?<br/>:null}{showTypeA}
                </span>
            )
        }
    }]
    //onClick={()=>{this.props.history.push('/platInvoice/inquiryDetail/' + record.uuids)}}
    ...
    // 查看详情
    seeDetails = ( uuids ) => {
        this.props.history.push(getUrlByParam("/platInvoice/inquiryDetail/" + uuids));
    }
