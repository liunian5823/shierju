...
class AuditForBasic extends React.Component {
  ...
  componentWillMount() {
    ...
    this.props.form.setFieldsValue({taxType: '1,2'});
    const uuids = this.props.match.params.uuids;//这里id是uuids
  ...
  getCompanyInfo = (id,workUuids) => {
    ...
    api.ajax("GET", "@/supplier/ecCompanySupplier/get", {
      ...
      r.data.isLicenseLong = (r.data.businessEndTime === 1 || r.data.businessEndTime === '1') ? true : false;
      if (r.data.businessEndTime == '1' || r.data.businessEndTime == 1) {
    ...
    this.legalType= r.data.legalType
      this.setState({
		...
		baseParams: {workOrdersType: 1,workOrdersTypeTwo: 2, companyId: r.data.companyId, workUuids: workUuids, license : r.data.businessLicense},
        ...
        companyType: r.data.type //供应商1、采购商0
      })
  ...
  initAuditModul = (key, isPassKey, phrase) => {
    ...
    if (isHas) {
      ...
      let initialValue = curObj.data[0]?curObj.data[0].id?curObj.data[0].id.toString():'':'';
      return [
        <Col span={8} key={isPassKey}>
          <FormItem {...this.formItemLayoutOther}>
            <RadioGroup
  ...
  handleSubmit = (status) => {
    if (status == 0) {
      ...
      this.props.form.validateFieldsAndScroll((errors, values) => {
        if (!!errors) {
			    console.log("er====ror:",errors)  
          return;
        }
        if(this.state.companyType==1 && !values.taxType) {//只有供应商才校验发票类型
          this.props.form.setFieldsValue({taxType: ''});
  ...
  currentSubmit = (status) => {
    ...
    //
    //
    let ary = ['name', 'provinceCode', 'cityCode', 'areaCode', 'address', 'factoryType', 'exportQualification', 'mainBusiness', 'legalPersonName', 'legalPersonIdType', 'legalPersonId', 'businessLicense', 'businessStartTime', 'businessEndTime', 'registeredCapital', 'taxType', 'companyBaseAccount', 'institutionalBank', 'taxpayerAttribute']
...
console.log(isApp, 'isappxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx')
    if (status == 0) {//保存
      this.props.form.validateFieldsAndScroll((errors, values) => {
        if (!!errors) {
          return;
		...
		values.taxType = this.props.form.getFieldValue('taxType');//this.arrToStr(this.props.form.getFieldValue('taxType'),null);
		if(values.taxType == ","){
			values.taxType = "";
		}
        ...
        if(values.approvalResult == 1 && isApp ) {
          ...
          api.ajax('POST', '@/supplier/ecUserApprovalLog/basicAudit?workOrdersId='+this.state.workId, {
            approvalType: 1,
            id: this.state.basicAuditInfo?this.state.basicAuditInfo.id:'',
            companyId: this.state.basicAuditInfo?this.state.basicAuditInfo.companyId:'',
            ...values,
            ...imgObj
          }).then(r => {
            if (!_this._isMounted) {
      ...
      this.props.form.validateFieldsAndScroll((errors, values) => {
		...
		values.taxType = values.taxType instanceof Array ? values.taxType.join(): values.taxType? values.taxType :'';
		console.log('values.expertiseArea:',values.mainBusiness)
		console.log('values.expertiseArea:',values.expertiseArea)
		console.log('values.supplyArea:',values.supplyArea)
        ...
        api.ajax('POST', '@/supplier/ecUserApprovalLog/basicAudit?workOrdersId='+this.state.workId, {
          ...
          ...values,
          ...imgObj,
          businessBrand: values.businessBrand? values.businessBrand: null ,
          businessProduct: values.businessProduct? values.businessProduct: null
        }).then(r => {
          if (!_this._isMounted) {
            return;
          }
    ...
    if (index == 1) {
      return [
        <Row gutter={16} style={{display: this.legalType == 1 ? 'none' : ''}} key={"legalPersonId"}>
          <Col span={12}>
            <FormItem {...formItemLayout} required={!this.state.stuss} label="法人身份证:">
              <Input type="text" maxLength='18' required={!this.state.stuss}
        ...
        <Row  gutter={16} style={{display: this.legalType == 1 ? 'none' : ''}} key={4}>
          <Col span={12}>
            <FormItem {...formItemLayout} required={!this.state.stuss} label="证件影像">
        ...
        <Row  gutter={16} style={{display: this.legalType == 1 ? 'none' : ''}} key={"legalPersonId"}>
          <Col span={12}>
            <FormItem {...formItemLayout} label="法人证件号码:">
              <Input type="text" maxLength='20'  required={!this.state.stuss}
                {...getFieldProps('legalPersonId',
                  {
                    rules: [
                      {
...
// 获取专业领域
  ...
  companyInfomation=()=>{
    ...
    window.open(url);
	  //window.open(window.location.href.split('#/')[0]+'#/' + 'qualification/basicInfomation?companyName='+this.state.companyName);
  }
	...
	renderSameCompany = () => {
	  ...
	  let columns = [
	    ...
	    },
	    {
	      ...
	      key: 'x',
	      render: (text, record) => {
	          return <AuthButton elmType="a" onClick={() => { this.handleToDetails(record) }}>详情</AuthButton>
	      }
	    }
	  ]
  ...
  render() {
	...
	const taxTypes = getFieldProps('taxType', {
	  rules: [
	    { required: this.state.companyType, message: '请选择发票类型' },
	  ],
    onChange: (e) => {
    ...
    const v = this.props.form.getFieldValue('approvalResult') == 1 ? false : true;
      return (
        <div>
          <Form>
            <Card className="mb10" title={<span><Badge count={1} style={{ backgroundColor: 'rgba(255,129,0,1)' }} /> 企业信息</span>} bordered={false}>
              <Row gutter={16}>
                <Col span={12}>
                  <FormItem required={!this.state.stuss} {...formItemLayout} label="企业类型">
                  ...
                  <FormItem required={!this.state.stuss} {...formItemLayout} label="企业级别">
                  ...
                  <FormItem required={!this.state.stuss} {...formItemLayout} label="上级单位">
                    <Input disabled type="text" maxLength='30'
                      {...getFieldProps('pname')}
                      placeholder="请输入公司名称" />
                  </FormItem>
                  <FormItem required={!this.state.stuss} {...formItemLayout} label="公司名称">
                    <Input disabled type="text" maxLength='30'
                      {...getFieldProps('name')}
                      placeholder="请输入公司名称" />
                  </FormItem>
                  <FormItem required={!this.state.stuss} {...formItemLayout} label="统一社会信用代码">
        ...
        <Row gutter={16} key="address">
          <Col span={12}>
            <FormItem {...formItemLayout} required={!this.state.stuss} label="机构所在地">
              <Col span={8}>
                <FormItem>
              ...
              <Col span={8}>
                <FormItem>
                  <Select {...getFieldProps("areaCode", {
                    rules: [
                      { required: !this.state.stuss, message: '请选择地区' },
                    ]
                  })}>
          ...
          <Col span={12}>
            {this.initAuditModul("exportQualification", "isPass_exportQualification", "phrase_exportQualification")}
              ...
              <Row gutter={16} style={{display: this.state.companyType==1 ? 'block' : 'none'}}>
                <Col span={12}>
                  <FormItem {...formItemLayout} required={!this.state.stuss} label="公司类型">
                    <RadioGroup
        ...
        <Row gutter={16} key={2}>
          <Col span={12}>
            <FormItem {...otherFormItemLayout} required={!this.state.stuss} label="执照有效期">
              <Col span={18}>
                <Col span={12}>
                  <FormItem>
                    <DatePicker
        ...
        <Row gutter={16} key={5}>
          <Col span={12}>
            <FormItem {...otherFormItemLayout} require={true} label="注册资金:">
              <Col span={17}>
                    ...
                    {...getFieldProps('registeredCapital',
                      {
                        rules: [
                          {
                            required: this.state.isRegisteredCapital? false:!this.state.isRegisteredCapital,
                            message: "请输入注册资本"
                          },
        ...
        <Row gutter={16} key={"businessLicensePath"}>
          <Col span={12}>
            <FormItem {...formItemLayout} required={!this.state.stuss} label="营业执照影像:">
        ...
        <Row gutter={16}>
          <Col span={12}>
            <FormItem {...formItemLayout} label="管理员是企业法人">
              <RadioGroup 
                disabled={true}
                {...getFieldProps('legalType',{initialValue: 1})}>
        ...
        <Row  gutter={16} style={{display: this.legalType == 1 ? 'none' : ''}} key={"legalPersonName"}>
          <Col span={12}>
            <FormItem {...formItemLayout} required={!this.state.stuss} label="法人姓名:">
          ...
          <Col span={12}>
            ...
            <Card style={{display: this.state.companyType==1 ? 'block' : 'none' }} className="mb10" title={<span><Badge count={3}
             style={{ backgroundColor: 'rgba(255,129,0,1)' }} /> 财务及税务信息</span>} bordered={false}>
              <Row gutter={16}>
                <Col span={12}>
                  <FormItem {...formItemLayout} required={this.state.companyType} label="银行基本账号">
                    <Input type="text" maxLength='20'
                      {...getFieldProps('companyBaseAccount',{
                        rules: [
                          {
                            required: this.state.companyType,
                            message: "请输入银行基本账号"
                          }
                        ]
                      })}
              ...
              <Row gutter={16}>
                <Col span={12}>
                  <FormItem {...formItemLayout} required={this.state.companyType} label="开户银行">
                    <Input type="text" maxLength='20'
            ...
            <Row gutter={16}>
                <Col span={12}>
                  <FormItem {...formItemLayout} required={this.state.companyType} label="纳税人资格属性">
                    <RadioGroup
                    ...
                    <div className="ant-form-explain" style={{display: this.taxTypeError?'':'none',color: '#f50', marginTop: 20}}>请选择发票类型</div>
                  </FormItem>
                </Col>
                <Col span={12}>
        ...
        <Row gutter={16}>
                <Col span={12}>
                  <FormItem {...formItemLayout} required={this.state.companyType} label="发票类型">
                    <CheckboxGroup
                      options={taxTypeData}
        ...
        <Row gutter={16}>
          ...
          <Col span={12}>
            ...
            <Card className="mb10" title={<span><Badge count={this.state.companyType ==1 ? 4 : 3}
            style={{ backgroundColor: 'rgba(255,129,0,1)' }} /> 经营信息</span>} bordered={false}>
              <Row gutter={16} key="mainBusiness" style={{display: this.state.companyType==1 ? 'block' : 'none'}}>
                <Col span={12}>
                  <FormItem {...formItemLayout} required={!this.state.stuss} label="主营类目">
        ...
        <Row gutter={16} key="expertiseArea">
          {/* expertiseArea 专业领域*/}
          <Col span={12}>
            <FormItem {...formItemLayout} required={!this.state.stuss}
              label="专业领域">
              <CheckboxGroup
                {...getFieldProps('expertiseArea', {
                rules: [
                  { required: !this.state.stuss, message: '请选择专业领域' },
                ],
              })}
        ...
        <Row gutter={16} key="areaArrStr" style={{display: this.state.companyType==1 ? 'block' : 'none'}}>
          <Col span={12}>
          <FormItem {...formItemLayout} required={this.state.companyType} label="供货区域">
            <Select
                {...getFieldProps("supplyArea", {
                  rules: [
                    { required: this.state.companyType, message: '请选择供货区域' },
                  ],
                })}
                multiple={true}
                placeholder="请选择供货区域"
          ...
          <Col span={12}>
            ...
            >
              <div 
                ...
                <Input placeholder='请输入经营品牌名称按回车或点击右侧按钮添加'
        ...
        <Row gutter={16} key="businessProduct" style={{display: this.state.companyType==1 ? 'block' : 'none'}}>
          <Col span={12}>
            <FormItem
              ...
              label='经营产品'
              >
              <div className={less.fullname_wrap}>
                <Input placeholder='请输入经营产品名称按回车或点击右侧按钮添加'
        ...
        <Row gutter={16}>
          ...
          <Col span={12}>
            <FormItem {...formItemLayout} label="审核人">
              <Input disabled={true} value={this._userInfo?this._userInfo.username:""}/>
            </FormItem>
          </Col>
              </Row>
        <Row gutter={16}>
          ...
          <Col span={12}>
          <FormItem {...formItemLayout} required={isRequired} label="审核结果">
            <RadioGroup
              {...getFieldProps('approvalResult',
                {
                  ...
                  onChange: (value) => {
                    this.setState({
                      stuss: value.target.value == 1? false: true
                    })
          ...
          <Col span={12}>
          <FormItem {...formItemLayout} label="审核意见" required={this.state.stuss}>
            <Input type="textarea" maxLength='1000' rows={5}
