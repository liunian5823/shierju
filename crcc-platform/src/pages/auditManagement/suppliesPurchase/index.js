...
class basicAudit extends React.Component {
  ...
  getStatisticNum = () => {
    api.ajax('POST', '@/platform/workOrders/getStatisticNum',{subType:"4"}).then(r => {
        ...
        },
        {
          ...
          render: (text, record) => {
              if(record.workOrdersState == 1){//待审核
                  return (
                      <span>
                        <span style={{cursor: 'pointer', color: '#2db7f5'}} onClick={() => this.handleToAudit(record.appUuids, record.companyId)}>审核</span>
  ...
  handleToExport = () => {
    ...
    window.open(
      ...
      '?selectType=sub&exportName=物资采购导出&' + params
    )
  }
