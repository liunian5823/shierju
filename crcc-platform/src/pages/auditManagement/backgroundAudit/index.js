...
class basicAudit extends React.Component {
  ...
  getStatisticNum = () => {
    ...
    api.ajax('POST', '@/platform/workOrders/getStatisticNum', {...paramMap}).then(r => {
		if (!this._isMounted) {
		  return;
		}
        ...
        },
        {
          ...
          render: (text, record) => {
            if (record.workOrdersState == "1" || record.workOrdersState == 1 ) {
              return (
				<span><AuthButton elmType="a" onClick={() => { this.handleToDetails(record.companyUuids+"!!"+record.uuids) }}>详情</AuthButton>
              ...
              return (
                <span>
                  <AuthButton elmType="a" onClick={() => { this.handleToDetails(record.companyUuids+"!!"+record.uuids) }}>详情</AuthButton>
                </span>
              )
            }
          }
  ...
  handleToAudit = (cuuids,wuuids) => {
    this.props.history.push(this.props.history.location.pathname + '/audit' + '/' + cuuids+'/'+wuuids);
  }
  ...
  handleToExport = () => {
    ...
    window.open(
      ...
      '?selectType=company&exportName=背景审核导出&' + params
    )
  }
