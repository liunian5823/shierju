...
class reject extends Component {
  ...
  handleConfirm() {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      if (err) return;
      let params = {...values}
      this.props.type == 3 ? 1:this.props.type
      params.financeHangId = this.props.financeHang.financeHangId
      params.financeHangUuids= this.props.financeHang.financeHangUuids
      ...
      api.ajax('GET','@/platform/approval/check', 
        {...params}
      ).then((res) => {
        if (res.data === true) {
          this.setState({
  ...
  render() {
    ...
    const phoneStr = this.props.userInfo.phone ? this.props.userInfo.phone.slice(0, 3) + 'XXXX' + this.props.userInfo.phone.slice(this.props.userInfo.phone.length - 4) : '000XXXX0000';
            ...
            <Spin spinning={false}>
              <Form>
                ...
                <RadioGroup onChange={this.onChange} value={this.state.value} {...getFieldProps('type', {
                    ...
                    <Radio key="a" value={1}>签章</Radio>
                    <Radio key="b" value={2}>手机验证</Radio>
                </RadioGroup>
                </FormItem>
