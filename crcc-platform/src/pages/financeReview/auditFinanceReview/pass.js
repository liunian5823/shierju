...
class pass extends Component {
  ...
  handleConfirm=()=> {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      if (err) return;
      let params = {...values}
      let type = this.props.listFinanceHangHandleOrders.type
      if(type == 3){
        type = 1
      }else if(type == 1){
        type =2
          ...
          <div className={less.reason_modal_container}>
            <Spin spinning={false}>
              <Form>
                ...
                <RadioGroup defaultValue="2" onChange={this.onChange} value={this.state.value} {...getFieldProps('type', {
                      initialValue:2,
                      rules: [
