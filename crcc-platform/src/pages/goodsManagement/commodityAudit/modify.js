  ...
  handelSubmit = () => {
    ...
    for(let index=0;index<this.props.modeData.length;index++){
      ...
      api.ajax("GET", "@/merchandise/ecGoodsSku/updateAmountWarning", {
        warning:this.props.modeData[index].warning,
        amount:this.props.modeData[index].amount,
        uuids:this.props.modeData[index].uuids,
      }).then(r => {
    ...
    if(this.props.modeData.length == 1){
      this.props.onOk(this._modalName, false, true);
      if(erlen!=0){
        Util.alert("更新失败");
      }
    ...
    else if(erlen != 0 && erlen == this.props.modeData.length){
      this.props.onOk(this._modalName, false, true);
      Util.alert("全部更新失败");
