...
class addContractTemplate extends React.Component{
    state = {
        loading: false,
        unitLevel:2,
        provinceList:[],
        ...
        contractNameTwo : "",
        contractVersion : "",
        dataTimeIsNotOptionalList:[],
        dataTimeIsNotOptional:null,
        dataTimeEffectivedIsNotOptionalList:[],
    }
    _isMounted = false;
    ...
    componentWillMount(){
        ...
        this.setTimeIsNotOptional();
        this.listContractTemplateEffectived();
    };

    componentWillUnmount(){
    };

    setTimeIsNotOptional=()=>{
        let that = this;
        ...
        api.ajax('GET', '!!/common/contractTemplate/listContractTemplateEffectiveNoToVoid',{
            ...
            if (r) {
                that.setState({
                    dataTimeIsNotOptionalList: r.data
                })
            } else {
                message.error("获得不可选日期段失败!!");
            }
        })
    }

    listContractTemplateEffectived=()=>{
        let that = this;
        let conversionOfContractTypes = 1;
        api.ajax('GET', '!!/common/contractTemplate/listContractTemplateEffectived',{
            ...
            if (r) {
                that.setState({
                    dataTimeEffectivedIsNotOptionalList: r.data
                })
            } else {
                message.error("获得不可选日期失败!!");
            }
        })
    }
    ...
    getEffectiveDeadline = (p) => {
        ...
        api.ajax('GET', '!!/common/contractTemplate/listContractTemplate',{
            ...
            if (r) {
                that.setState({
                    effectiveTimeStr: moment(new Date(r.data[0].endTime)).format('YYYY-MM-DD'),
                    effectiveTimeValue: r.data[0].endTime
                })
            } else {
    ...
    handleSubmit=()=>{
        let that = this;
        this.getCurrentBean().then(function(data){
            ...
            that.props.form.validateFieldsAndScroll((errors, values) => {
                    ...
                    if (data.length<=0) {
                        ...
                        param.endTimeStr = moment(new Date(that.state.endValue.getTime())).format('YYYY-MM-DD');
                        param.signTimeStr = that.state.signTimeStr;
                        param.delFlag = 0;
                        param.contractClass = 1;
                        ...
                        api.ajax("POST", "!!/common/contractTemplate/insertContractTemplate", {
                            ...
                            that.props.history.push("/contract/contractTemplate");
                        }).catch((r) => {
                            that.setState({
                                loading: false
                            })
    ...
    preview=()=>{
        this.props.form.validateFieldsAndScroll((errors, values) => {
            ...
            param.endTimeStr = moment(new Date(this.state.endValue.getTime())).format('YYYY-MM-DD');
            ...
            api.ajax("POST", "!!/common/contractTemplate/insertContractTemplate", {
                ...
                if(r){
                    window.open(SystemConfig.systemConfigPath.dfsPathUrl(r.data));
                }
            }).catch((r) => {
                this.setState({
                    loading: false
     ...
     * 选截止
     */
    selectionCutOff=(startValueTime,endValue,now)=>{
        let that = this;
        let dataTimeIsNotOptional = startValueTime < now;
        if (that.state.dataTimeIsNotOptionalList.length>0){
            for (let i = 0; i < that.state.dataTimeIsNotOptionalList.length; i++){
                let dataTimeIsNotOptionalOne = that.state.dataTimeIsNotOptionalList[i];
                let effectiveTime = new Date(dataTimeIsNotOptionalOne.effectiveTime);
                let endTime = new Date(dataTimeIsNotOptionalOne.endTime);
                if (endValue > endTime){
                    return endTime
                }
            }
     ...
     * 选生效
     */
    electionTakesEffect=(endValueTime,startValue,now)=>{
        let that = this;
        let dataTimeIsNotOptional = endValueTime < now;
        if (that.state.dataTimeIsNotOptionalList.length>0){
            for (let i = 0; i < that.state.dataTimeIsNotOptionalList.length; i++){
                let dataTimeIsNotOptionalOne = that.state.dataTimeIsNotOptionalList[i];
                let effectiveTime = new Date(dataTimeIsNotOptionalOne.effectiveTime);
                let endTime = new Date(dataTimeIsNotOptionalOne.endTime);
                if (startValue > effectiveTime){
                    if (i>0){
                        return that.state.dataTimeIsNotOptionalList[i-1].effectiveTime
                    } else {
                        return null
                    }
                }
            }
            return that.state.dataTimeIsNotOptionalList[that.state.dataTimeIsNotOptionalList.length-1].effectiveTime;
        }
        return null;
    }
    ...
    disabledStartDate=(startValue)=> {
        ...
        let startValueTime = startValue.getTime()
        startValueTime = startValueTime - startValueTime % 86400000 - 28800000
        // 没有结束时间
        if (!startValue || !that.state.endValue) {
            let dataTimeIsNotOptional = startValueTime < now;
            let dataTimeEffectivedIsNotOptionalList = that.state.dataTimeEffectivedIsNotOptionalList;
            if (dataTimeEffectivedIsNotOptionalList.length > 0) {
                let effectiveTime = new Date(dataTimeEffectivedIsNotOptionalList[0].effectiveTime);
                let endTime = new Date(dataTimeEffectivedIsNotOptionalList[0].endTime);
                // endTime.setDate(endTime.getDate() + 1);
                dataTimeIsNotOptional = effectiveTime >= startValueTime || startValueTime >= endTime || dataTimeIsNotOptional
            }
            for (let i = 0; i < that.state.dataTimeIsNotOptionalList.length; i++){
                let dataTimeIsNotOptionalOne = that.state.dataTimeIsNotOptionalList[i];
                let effectiveTime = new Date(dataTimeIsNotOptionalOne.effectiveTime);
                let endTime = new Date(dataTimeIsNotOptionalOne.endTime);
                endTime.setDate(endTime.getDate()+1);
                dataTimeIsNotOptional  = startValueTime == dataTimeIsNotOptionalOne.effectiveTime || dataTimeIsNotOptional
            }
            return dataTimeIsNotOptional;
        }else {

            let endValue = that.state.endValue
            let endValueTime = new Date(endValue);
            let endValueGetTime = endValueTime.getTime() - endValueTime.getTime() % 86400000
            let dataTimeIsNotOptional = startValueTime < now || endValueGetTime < startValueTime;
            let dataTimeEffectivedIsNotOptionalList = that.state.dataTimeEffectivedIsNotOptionalList;
            if (dataTimeEffectivedIsNotOptionalList.length > 0) {
                let effectiveTime = new Date(dataTimeEffectivedIsNotOptionalList[0].effectiveTime);
                let endTime = new Date(dataTimeEffectivedIsNotOptionalList[0].endTime);
    ...
    disabledEndDate(endValue) {
        ...
        let now = new Date();
        now.setDate(now.getDate()-1);
        let endValueTime = endValue.getTime()
        endValueTime = endValueTime - endValueTime % 86400000 - 28800000
        if (!endValue || !this.state.startValue) {
            ...
            if (dataTimeEffectivedIsNotOptionalList.length > 0) {
                ...
                let endTime = new Date(dataTimeEffectivedIsNotOptionalList[0].endTime);
                // endTime.setDate(endTime.getDate() + 1);
                dataTimeIsNotOptional = endValueTime <= endTime || dataTimeIsNotOptional
            }
            return dataTimeIsNotOptional;
        }else {

            let startValue = this.state.startValue
            let startValueTime = new Date(startValue);
            let startValueGet = startValueTime.getTime() - startValueTime.getTime() % 86400000
            let dataTimeIsNotOptional = endValueTime < now || endValueTime < startValueGet;
            let dataTimeEffectivedIsNotOptionalList = that.state.dataTimeEffectivedIsNotOptionalList;
            if (dataTimeEffectivedIsNotOptionalList.length > 0) {
                let effectiveTime = new Date(dataTimeEffectivedIsNotOptionalList[0].effectiveTime);
                let endTime = new Date(dataTimeEffectivedIsNotOptionalList[0].endTime);
    ...
    onStartChange=(value)=> {
        if(value){
            ...
            signTime.setDate(signTime.getDate()-30);
    ...
    render(){
        ...
        return(
            <div>
                <Card bordered={false} title="发布次年协议">
                <Form>
                    ...
                    <BaseAffix>
                        ...
                        <Button ghost style={{backgroundColor: "#FFF",marginRight: "10px"}} loading={this.state.loading} onClick={this.preview}>预 览</Button>
                        <Button ghost style={{backgroundColor: "#FFF",marginRight: "10px"}} onClick={()=>{
                            this.props.history.push("/contract/contractTemplate");
                        }}>返 回</Button>
                    </BaseAffix>
                </Form>
                </Card>
...
export default Form.create()(addContractTemplate)
