...
class ModalFormComponent extends React.Component {
  ...
  uploadProps = (info) => {
    ...
    if (info.file.status === 'done') {
      ...
      if(info.file.response.code == '200'){
        Util.alert('导入成功', { type: 'error' })
      }else{
        Util.alert( info.file.response.msg, { type: 'success' })
      }
  ...
  handleToExport = () => {
    window.open(
      ...
      '/api' +
      '/supplier/ecBlacklistCompany/downBlackListFile'
    )
  }
          ...
          <Form>
            <FormItem {...formItemLayout}>
              <Upload
                ...
                action={SystemConfig.configs.axiosUrl + '/supplier/ecBlacklistCompany/batchBackCompany'}
