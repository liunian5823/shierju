...
class ModalForm extends React.Component {
  ...
  _userInfo = null
  _visible = false
  _modalName = "ModalreSubAdminShow"
  _clientId = null
  _clientTime = null
  params = {}
  ...
  componentDidUpdate() {
    if (this._visible != this.props.visible) {
      ...
      this.params = { ...this.params, companyId: this.props.accountInfo.companyId,subPlatformId:this.props.subPlatformId };
      this.getImg();
    }
  }
  ...
  uploadProps = (info) => {
    ...
    this.props.form.setFieldsValue({ authorizationPath: fileList })
  }
  ...
  handelSubmit = () => {
    ...
    if (currentStep == 0) {
      ...
      validateList = ['newUsername', 'gender', 'citizenCode', 'newPhone', 'newPhoneCode', 'email', 'provinceCode','cityCode','areaCode','address','remark', 'authorizationPath']
      this.props.form.validateFields(validateList, (errors, values) => {
        if (!!errors) {
          return;
        }
  ...
  toOneSubmit = (values) => {
    ...
    api.ajax('GET', '@/sso/loginControl/validateUsernameBySubPlatform', {
      ...this.params,
      type: 2,
      ...values,
    }).then(r => {
  ...
  toSubmit = (values) => {
	...
	const params = {};
	params.isOldUser = this.state.isOldUser;
	params.companyId = this.props.companyId;
	params.companyName = this.props.companyName;
	params.subId = this.props.subPlatformId;
	params.smsCode= values.newPhoneCode;
	params.ecUserModel = {
	...
	let clariPath = values.authorizationPath;
	if (clariPath) {
	    params.authorizationPath = clariPath.fileList && clariPath.fileList[0] ? clariPath.fileList[0].response.data : '';
	    params.authorizationName = values.authorizationPath.file.name;
	}
	console.log(params)
    api.ajax("POST", "@/sso/ecUser/updateSubAdminByPlatform", {
      ...params
    }).then(r => {
      if (!this._isMounted) { return }
      this.setState({
      ...
      this.props.onOk(this._modalName, this.props.subPlatformId, false, this.state.result);
    }).catch(r => {
      Util.alert(r.msg, { type: "error" })
      this.setState({
        _loading: false,
		    ...
		    <Col span="22">
		      ...
		      >
		        <Row gutter={8}>
		          ...
		          <Col span={8}>
		            <FormItem>
		              <Select placeholder='请选择市/区'
		                  style={{
		                      width: 100
		                  }} {...getFieldProps('cityCode', { 
							  rules: [
							    { required: true, message: '请选择市/区' }
							  ],
            ...
            <Col span="22">
              ...
              >
                <Upload
                  ...
                  {...getFieldProps('authorizationPath', {
                    rules: [
