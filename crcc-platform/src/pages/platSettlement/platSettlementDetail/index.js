import { Tabs, Row, Col, Button, Table, Card,Form } from 'antd';
...
class PlatSettlementDetail extends React.Component{
    ...
    getSettlementDetails=(id)=>{
        let _this = this;
        api.ajax('GET','!!/financial/platSettlementController/queryEcSettlementById',{
            id:id
        }).then(r => {
            if(!_this._isMounted){
                return;
    ...
    handleCancel = () => {
        this.props.history.push('/platSettlement/platSettlementList');
    }
            ...
            <div>
                    ...
                    <Row>
                        <Col span={12}>
                            <BaseDetails title="供应商">
                                {this.state.dataSource.sellerName}
                            </BaseDetails>
                        </Col>
                        <Col span={12}>
                ...
                <Card bordered={false} className="mb10" title="结算单日志" >
                    <Row>
                        <Col span={24}>
                            <BaseDetails title="">
                                {this.state.dataSource.orderList.length == 0?"":this.state.dataSource.orderList[0].comments}
                            </BaseDetails>
                        </Col>
                    </Row>
                </Card>
