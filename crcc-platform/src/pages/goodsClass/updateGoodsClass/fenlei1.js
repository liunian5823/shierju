...
const searchInput = "searchInput1";
export default class Fenlei extends React.Component {
	...
	//将当前的商品分类传给父组件
	handPublish = () =>{
		this.props.handleGetGoodsClassNew(this.state.tempGoodsClass);
	}
	...
	getSearchInput = () =>{
		//获取searchInput输入的内容
		let _search_input = $("#searchInput1").val();
	...
	render(){
		...
		{this.handPublish()}
		return(
			<div className={less.leftClass}>
				<div className={less["cy-fl"]}>
					<Card bordered={false} title="新分类选择" extra={this.searchGoodsClass()}>
