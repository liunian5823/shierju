...
// }
  ...
  handleTabChange = (key) => {
    ...
    if (0 == key) {
      //铁建商城
      this.baseParams.subId = 0
  } else if (3 == key) {
      //物资销售
      this.baseParams.subId = 3
  } else if (4 == key) {
      //物资采购
      this.baseParams.subId = 4
  } else if (6 == key) {
      // 物资循环
      this.baseParams.subId = 6
  } else if (9 == key) {
      //招标采购
      this.baseParams.subId = 9
  }else if (1 == key) {
      //全部
      this.baseParams.subId = undefined
  }
  this.activeTab = key;
  this.reloadTableData();
  }
      ...
      <div>
        ...
        <Card bordered={false} title="消息通知" extra={<Button type="primary" onClick={this.operation}>批量操作</Button>}>
          ...
          <Tabs className={less.tabs} tabBarExtraContent={operations} onChange={this.handleTabChange}>
            <TabPane tab="全部" key="1" className={less.tabplane}>
              <div className={less.tabplane}>
                <BaseTableCenter
                  url="@/platform/platformMessage/pageMessageList"
