...
class PurchaseAccount extends React.Component {
        constructor(props) {
        super(props);
            this.state = {
                ...
                timeS: true,
                sumPrice: 0,
                inquiryReuseCount: null,    //循环物资采购商数量
                inquiryBidCount: null,      //电子招投标采购商数量
            }
        }
        componentWillMount() {
            this.getData();
        ...
        //获取采购员账号数量
        getData = params => {
            api.ajax('GET',"@/statistics/purchaseStatistic/buyerStatistic", {
                ...params
            }).then(r => {
                this.setState({
                    dataInfo: r.data.rows,
                    sumPrice: r.data.sum.totalBuyer,
                    inquiryReuseCount: r.data.inquiryReuseCount,
                    inquiryBidCount: r.data.inquiryBidCount,
                    loading: false
                })
            });
        }
        //获取采购员账号数量
        getTotalM = params => {
            api.ajax('GET',"@/statistics/yearStatistic/selectInquiryCountSum", {
                ...params
            }).then(r => {
                this.setState({
                    dataTotal: r.data,
        ...
        render(){
            ...
            const { anyYears, dataInfo, loading, monthS, selS, timeS ,sumPrice, inquiryReuseCount, inquiryBidCount } = this.state;
            return(
                <div>
                    <Card bordered={false} className="cards">
                        ...
                        <h3 style={{marginBottom: "15px",zIndex: 999, display : "inline-block", position:"relative"}}>采购账号总数量: {sumPrice + inquiryReuseCount + inquiryBidCount} &nbsp; | &nbsp;询竞价采购商数量: {sumPrice}&nbsp; | &nbsp;循环物资采购商数量: {inquiryReuseCount}&nbsp; | &nbsp;电子招投标采购商数量: {inquiryBidCount}&nbsp;</h3>
                        <Table
                            dataSource={dataInfo}
...
export default Form.create()(PurchaseAccount);