...
class YearJZLStatisticByJu extends React.Component {
        constructor(props) {
        super(props);
            this.state = {
        ...
        //获取撮合交易金额
        getData = params => {
            api.ajax('GET',"@/statistics/yearStatistic/selectJZLByJu", {
                ...params
            }).then(r => {
                this.setState({
        ...
        //获取撮合交易金额总金额
        getTotalM = params => {
            api.ajax('GET',"@/statistics/yearStatistic/selectJZLSumByJu", {
                ...params
            }).then(r => {
                this.setState({
                    dataTotal: r.data,
        ...
        columns = () => {
            return [
                ...
                },
                {
                    ...
                    width: 250,
                    render: (text, record)=>{
                        let _text = JSON.stringify(text*100);
                        if(_text.indexOf(".") > -1) {
                            _text = JSON.parse(_text).toFixed(2);
                        }
                        return <span title={text}>{text ? (_text + "%") : "-"}</span>
                    }
                },
            ]
        }
            ...
            return(
                <div>
                    <Card bordered={false} className="cards">
                        ...
                        <h3 style={{marginBottom: "15px",zIndex: 999, display : "inline-block", position:"relative"}}>报价平均总金额: {dataTotal.avgvalsum ? dataTotal.avgvalsum : "0" }</h3>
                        <h3 style={{marginBottom: "15px",zIndex: 999, display : "inline-block", marginLeft : "20px", position:"relative"}}>中标总金额: {dataTotal.orderpricesum ? dataTotal.orderpricesum : "0" }</h3>
                        <h3 style={{marginBottom: "15px",zIndex: 999, display : "inline-block", marginLeft : "20px", position:"relative"}}>节资总金额: {dataTotal.jzjesum ? dataTotal.jzjesum : "0" }</h3>
                        <h3 style={{marginBottom: "15px",zIndex: 999, display : "inline-block", marginLeft : "20px", position:"relative"}}>节资率: {dataTotal.jzlsum ? dataTotal.jzlsum : "0" }</h3>
                        <Table
                            dataSource={dataInfo}
...
export default Form.create()(YearJZLStatisticByJu);