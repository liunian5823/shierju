...
class YearStatisticByJu extends React.Component {
        constructor(props) {
        super(props);
            this.state = {
        ...
        //获取平台采购量
        getData = params => {
            api.ajax('GET',"@/statistics/yearStatistic/selectYearPurchaseByJu", {
                ...params
            }).then(r => {
                this.setState({
        ...
        //获取平台采购总金额
        getTotalM = params => {
            api.ajax('GET',"@/statistics/yearStatistic/selectYearPurchaseSumByJu", {
                ...params
            }).then(r => {
                this.setState({
                    dataTotal: r.data,
            ...
            return(
                <div>
                    <Card bordered={false} className="cards">
                        ...
                        <h3 style={{marginBottom: "15px",zIndex: 999, display : "inline-block", position:"relative"}}>未成交的询价单最低报价金额: {dataTotal.quotationSum ? dataTotal.quotationSum : "0" }</h3>
                        <h3 className="titleAll">订单总金额: {dataTotal.orderSum ? dataTotal.orderSum : "0" }</h3>
                        <h3 className="titleAll">中标总金额: {dataTotal.informationSum ? dataTotal.informationSum : "0" }</h3>
                        <h3 className="titleAll">采购规模: {dataTotal.cggmSum ? dataTotal.cggmSum : "0" }</h3>
                        <Table
                            dataSource={dataInfo}
...
export default Form.create()(YearStatisticByJu);