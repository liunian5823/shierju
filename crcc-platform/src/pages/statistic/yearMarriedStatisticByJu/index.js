...
class YearMarriedStatisticByJu extends React.Component {
        constructor(props) {
        super(props);
            this.state = {
        ...
        //获取撮合交易金额
        getData = params => {
            api.ajax('GET',"@/statistics/yearStatistic/selectMarriedDealByJu", {
                ...params
            }).then(r => {
                this.setState({
        ...
        //获取撮合交易金额总金额
        getTotalM = params => {
            api.ajax('GET',"@/statistics/yearStatistic/selectMarriedDealSumByJu", {
                ...params
            }).then(r => {
                this.setState({
                    dataTotal: r.data,
            ...
            return(
                <div>
                    <Card bordered={false} className="cards">
                        ...
                        <h3 style={{marginBottom: "15px",zIndex: 999, display : "inline-block", position:"relative"}}>订单总金额: {dataTotal.orderSum ? dataTotal.orderSum : "0" }</h3>
                        <h3 style={{marginBottom: "15px",zIndex: 999, display : "inline-block", marginLeft : "20px", position:"relative"}}>中标总金额: {dataTotal.bidSum ? dataTotal.bidSum : "0" }</h3>
                        <h3 style={{marginBottom: "15px",zIndex: 999, display : "inline-block", marginLeft : "20px", position:"relative"}}>平台撮合交易总金额（GMV）: {dataTotal.chjyzesum ? dataTotal.chjyzesum : "0" }</h3>
                        <Table
                            dataSource={dataInfo}
...
export default Form.create()(YearMarriedStatisticByJu);