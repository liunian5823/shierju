...
class PurchasedItems extends React.Component {
        ...
        //获取采购项目部数量
        getData = params => {
            api.ajax('GET',"@/statistics/purchaseStatistic/projectDeptStatistic", {
                ...params
            }).then(r => {
                this.setState({
                    dataInfo: r.data.rows,
                    sumPrice: r.data.sum.totalDep,
                    loading: false
                })
            });
        }
        ...
        render(){
            ...
            const { anyYears, dataInfo, loading, monthS, selS, timeS, sumPrice } = this.state;
            return(
                <div>
                    <Card bordered={false} className="cards">
                        ...
                        <h3 style={{marginBottom: "15px",zIndex: 999, display : "inline-block", position:"relative"}}>采购项目部总数量: {sumPrice}</h3>
                        <Table
                            dataSource={dataInfo}
...
export default Form.create()(PurchasedItems);