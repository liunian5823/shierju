...
//订单状态
const _ORDERSTAUS = baseService.orderStatusBuyerNew;
            ...
            render: (text, record) => (
                <span className="reuse_baseButtonGroup">
                  <AuthButton elmType="a" style={{display: 'block'}} onClick={() => {
                    ...
                    {
                        this.getAuthButton([30], record.status)
                            ? <AuthButton elmType="a" style={{display: 'block'}} onClick={() => {
                                this.handleToOrder(record)
                            }}>确认榜单</AuthButton> : null
                    }
              </span>
            )
        }
