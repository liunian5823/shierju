...
// }
  ...
  delete=()=>{
    ...
    confirm({
        ...
        onOk() {
            ...
            let uuids = {str}
            params.uuids = uuids;
            api.ajax("GET", "@/message/ecMessage/deleteListByuuids", {uuids:str}
            ).then((r) => {
                if (r.code != 200) {
                    message.error(r.msg)
                } else {
  ...
  read=()=>{
    ...
    confirm({
        ...
        onOk() {
            ...
            api.ajax("GET", "@/message/ecMessage/readOnlyListByuuids", {uuids:str}
            ).then((r) => {
                if (r.code != 200) {
                    message.error(r.msg)
                } else {
  ...
  handleToDetails = (record) => {
    ...
    api.ajax("GET", "@/message/ecMessage/queryByuuids", {uuids:record.uuids}
    ).then((r) => {
        if (r.code != 200) {
            message.error(r.msg)
        } else {
  ...
  handleToDelete=(record)=>{
    api.ajax("GET", "@/message/ecMessage/deleteByuuids", {uuids:record.uuids}
            ).then((r) => {
                if (r.code != 200) {
                    message.error(r.msg)
                } else {
  ...
  handleToRead=(record)=>{
    api.ajax("GET", "@/message/ecMessage/readOnly", {uuids:record.uuids}
            ).then((r) => {
                if (r.code != 200) {
                    message.error(r.msg)
                } else {
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Card id="btn_bottom" className={less.btns} style={{display:this.state.bottom ? 'block' :'none',position:'fixed',bottom:'0px',width:'1010px',zIndex:100}}>
